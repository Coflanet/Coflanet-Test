...
> ✅ **구현 완료(commit `a45605a`, CI analyze+test 통과)**: A1(홈 원두 카드→상세), B1(원두 상세 네이버 판매링크 열기), 커뮤니티 탭 숨김, 장바구니 아이콘 숨김. **쇼핑 탭은 유지(취향 추천 페이지)**.
> ⬜ 남음: A2(매칭 칩), B3(온보딩 더보기), C1(홈 상품 카드↔쇼핑 페이지 — 쇼핑 화면 신설 후).
