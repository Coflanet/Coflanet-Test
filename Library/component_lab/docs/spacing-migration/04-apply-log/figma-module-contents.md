...
**입력**: `03-mapping/figma-node-to-token.csv` 중 `group=module-contents AND status=READY` (11,107행 / 5,933 distinct node)
...
11,107 binding을 23개 청크로 분할 (use_figma 코드 50KB 한계 대응, 각 ≤500 tuples).
...
|---|--:|
| `AppSpacing.s2` | 1,301 |
| `AppSpacingSemantic.inlineMd` | 1,053 |
| `AppSpacing.s4` | 959 |
| `AppSpacing.s10` | 881 |
| `AppSpacingSemantic.inlineSm` | 873 |
| `AppSpacing.s6` | 723 |
| `AppSpacing.s3` | 639 |
| `AppSpacingSemantic.insetXl` | 588 |
| `AppSpacing.s8` | 524 |
| `AppSpacing.s20` | 453 |
...
| reason | count |
|---|--:|
| directional padding (palette direct) | 4,421 |
| value-based stack/inline (canonical) | 2,939 |
| palette direct (no semantic match for 10px) | 841 |
| inset-squish Xs (V2/H6) | 772 |
| inset (4-side equal 24px) | 588 |
| inset (4-side equal 4px) | 340 |
| 4-side equal 6px (no semantic inset match) | 304 |
| 4-side equal 20px (no semantic inset match) | 152 |
| palette direct (no semantic match for 6px) | 132 |
| inset (4-side equal 8px) | 116 |
...
`group=module-contents` 의 `status=NEEDS_REVIEW` 행은 본 작업 범위 밖. 별도 사람 검수 (`03-mapping/REVIEW_QUEUE.md`).
