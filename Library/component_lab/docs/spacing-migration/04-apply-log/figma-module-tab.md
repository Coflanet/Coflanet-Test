# Phase 4 — Figma module-tab 적용 로그

**대상 파일**: `q7yBPcHrid1CGQqFWEPwnR` (메인)
**대상 그룹**: `group=module-tab`
**입력**: `03-mapping/figma-node-to-token.csv` 중 `group=module-tab AND status=READY` (5,123행 / 2,589 distinct node)
**plan**: `03-mapping/plans/figma-module-tab.json`
...
5,123 binding을 11개 청크로 분할 (use_figma 코드 50KB 한계 대응, 각 ≤500 tuples).
...
| `AppSpacing.s6` | 309 |
| `AppSpacing.s4` | 308 |
| `AppSpacingSemantic.inlineXs` | 218 |
| `AppSpacing.s20` | 160 |
| `AppSpacingSemantic.inlineSm` | 115 |

### reason (상위 10)

| reason | count |
|---|--:|
| directional padding (palette direct) | 2,831 |
| palette direct (no semantic match for 10px) | 659 |
| value-based stack/inline (canonical) | 443 |
| 4-side equal 9px (no semantic inset match) | 320 |
| 4-side equal 6px (no semantic inset match) | 272 |
| inset-squish Xs (V2/H6) | 156 |
| 4-side equal 2px (no semantic inset match) | 112 |
| palette direct (no semantic match for 3px) | 97 |
| inset (4-side equal 24px) | 96 |
| palette direct (no semantic match for 20px) | 52 |
...
`group=module-tab` 의 `status=NEEDS_REVIEW` 행은 본 작업 범위 밖. 별도 사람 검수 (`03-mapping/REVIEW_QUEUE.md`).
